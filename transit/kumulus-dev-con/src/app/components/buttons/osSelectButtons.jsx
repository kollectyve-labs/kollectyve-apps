...
function SelectableButtons({ activeOS, activeVersion, onSelectionChange }) {
  const [isOpen, setIsOpen] = useState(false);
  const [localSelectedDistribution, setLocalSelectedDistribution] = useState(activeOS || 'Ubuntu');
  const [localSelectedVersion, setLocalSelectedVersion] = useState(activeVersion || ''); 

  // Sync local state with props when they change
  useEffect(() => {
    if (activeOS) setLocalSelectedDistribution(activeOS);
  }, [activeOS]);

  useEffect(() => {
    if (activeVersion) setLocalSelectedVersion(activeVersion);
  }, [activeVersion]);
  ...
  const versions = {
    Ubuntu: ['20.04 LTS', '22.04 LTS', '23.04 LTS'],
  ...
  const handleSelectVersion = (version) => {
    ...
    setIsOpen(false);
    onSelectionChange(localSelectedDistribution, version); // Pass the selected version and OS back to parent
  };

  // Handle selecting a distribution
  const handleSelectDistribution = (distribution) => {
    if (distribution !== 'Ubuntu') return; // Disable selection for Debian & Fedora
    setLocalSelectedDistribution(distribution);
    setLocalSelectedVersion(''); // Reset version when distribution changes
    setIsOpen(false);
    onSelectionChange(distribution, ''); // Pass the distribution and empty version to parent
  };

  // Get the list of versions to display based on the selected distribution
  const getVersionsToDisplay = () => {
    if (localSelectedDistribution) {
      return versions[localSelectedDistribution];
    }
    return [];
  };

  return (
    <div className=" flex pt-12 h-[270px] flex-col">
      ...
      <div className="flex gap-4">
        <button
          className={`w-[110px] h-[110px] text-[1em] font-semibold text-black flex flex-col items-center rounded-lg border-2 shadow-sm
            ...
            hover:border-[#6200ee] hover:scale-105 ${
              localSelectedDistribution === 'Ubuntu' ? 'border-[#6200ee]' : 'border-gray-400'
            }`}
        ...
        <div className="flex flex-col">
          <button
            className={` group relative w-[110px] h-[110px] text-[1em] font-semibold text-black flex flex-col items-center gap-3 rounded-lg border-2 shadow-sm 
            transition-all ease-in-out duration-300 
            hover:border-[#6200ee] hover:scale-105 ${
              localSelectedDistribution === 'Debian' ? 'border-[#6200ee]' : 'border-gray-400'
            }`}
            onClick={() => handleSelectDistribution('Debian')}
          >
            <img className="w-[55px] h-[55px] mx-auto mt-4" src="/images/under-construction.png" alt="Debian logo" />
            <div className="mx-auto">Debian</div>
            <span className="absolute left-1/2 -translate-x-1/2 top-full mt-2 px-3 py-1 bg-gray-800 text-white text-sm rounded-lg shadow-md whitespace-nowrap z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
              In the works
          </span>
          </button>
        </div>
        ...
        <button
          className={` group relative w-[110px] h-[110px] text-[1em] font-semibold text-black flex flex-col items-center gap-3 rounded-lg border-2 shadow-sm 
            transition-all ease-in-out duration-300 
            hover:border-[#6200ee] hover:scale-105 ${
              localSelectedDistribution === 'Fedora' ? 'border-[#6200ee]' : 'border-gray-400'
            }`}
          onClick={() => handleSelectDistribution('Fedora')}
        >
          <img className="w-[55px] h-[55px] mx-auto mt-4" src="/images/under-construction.png" alt="Fedora logo" />
    ...
    In the works
      ...
      {localSelectedDistribution && (
        <div className="mt-4">
          ...
          >
            {localSelectedVersion || `Select a ${localSelectedDistribution} Version`}
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
              <path d="M12 17.414 3.293 8.707l1.414-1.414L12 14.586l7.293-7.293 1.414 1.414L12 17.414z" />
            </svg>
          </button>
          {isOpen && (
            <div className="w-[365px] mt-1 absolute bg-white border  border-gray-300 rounded-md shadow-lg z-10">
              <div className="py-1">
                {/* Display versions of the selected distribution */}
                {getVersionsToDisplay().map((version) => (
                  <div
                    key={version}
                    className="px-4 py-2 cursor-pointer h-[40px] hover:bg-gray-100 z-10"
